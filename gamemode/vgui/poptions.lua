function MakepOptions()
	...
	local check = vgui.Create("DCheckBoxLabel", Window)
	check:SetText("항상 좀비로 시작하기")
	check:SetConVar("zs_alwaysvolunteer")
	check:SizeToContents()
	list:AddItem(check)
	
	local check = vgui.Create("DCheckBoxLabel", Window)
	check:SetText("보스 좀비가 되지 않기")
