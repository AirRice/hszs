...
function ENT:DrawTranslucent()
	...
	if !self.ShouldDrawLaser then return end
...
net.Receive("laserturret", function(len)
	...
	if id == 1 then
		...
		if IsValid(ent) then
			ent.OriginalDrawTranslucent = ent.OriginalDrawTranslucent or ent.DrawTranslucent
			ent.ShouldDrawLaser = false
			ent.DrawTranslucent = function(self)
				ent.OriginalDrawTranslucent(self)
				
				render.SetMaterial(matBeam)
				local col = VectorRand():ToColor()
				col.a = math.random(60, 90)
				render.DrawBeam(src, hitpos, math.Rand(30, 50), 0, 1, col)
			end
		end
	else
		local ent = net.ReadEntity()
		if IsValid(ent) then
			ent.ShouldDrawLaser = true
			ent.DrawTranslucent = ent.OriginalDrawTranslucent or ent.DrawTranslucent
		end
	end
	chat.AddText(tostring(id))
end)