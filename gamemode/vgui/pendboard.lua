...
function MakepEndBoard(winner)
	...
	if localwin then
		...
		heading = EasyLabel(frame, "승리!", "ZSHUDFont", COLOR_CYAN)
	else
		surface.PlaySound("ambient/levels/citadel/strange_talk"..math.random(3, 11)..".wav")
		heading = EasyLabel(frame, "패배.", "ZSHUDFont", COLOR_RED)
	end
	...
	local subheading
	if localwin then
		subheading = EasyLabel(frame, "인간들은 이 순간을 위해 살아남았다.", "ZSHUDFontSmall", COLOR_WHITE)
	else
		subheading = EasyLabel(frame, "언데드 군단이 더욱 더 강력해졌다.", "ZSHUDFontSmall", COLOR_LIMEGREEN)
	end
