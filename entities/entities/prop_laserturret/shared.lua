...
function ENT:GetManualControl()
	...
	if owner:IsValid() and owner:Alive() and owner:Team() == TEAM_HUMAN then
		...
		if wep:IsValid() and wep:GetClass() == "weapon_zs_laserturretcontrol" and wep.GetTurret and wep:GetTurret() == self and not wep:GetDTBool(0) then
			return true
		end
	end
