...
function GM:PlayerSpawn(pl)
	...
	if pl:Team() == TEAM_UNDEAD then
		...
		if self.OutnumberedHealthBonus <= numundead or classtab.Boss then
			...
			pl:SetHealth(classtab.Health * 2.0)
		end
		...
		if self.ZombieEscape then
			...
			elseif pl.m_PreRedeem then
				if self.RedeemLoadout then
					...
					pl:Give("weapon_zs_boardpack")
					pl:Give("weapon_zs_hammer")
					pl:Give("weapon_zs_blaster")
					pl:GiveAmmo(600, "pistol", true)
					pl:GiveAmmo(20, "SniperRound", true)
					pl:GiveAmmo(300, "Battery", true)
					pl:GiveAmmo(80, "buckshot", true)
					pl:Give("weapon_zs_manhack")
					pl:Give("weapon_zs_drone")
				end
			end
		end
