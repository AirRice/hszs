		...
		if IsValid(phys) and IsValid(target) then
			...
			if dist <= self.TraceHumanRadius then
				...
				if tr.Hit and tr.Entity == target then
					...
					phys:AddVelocity(dir * math.max(self.MaxHumanVel * (1 - (dist / self.TraceHumanRadius)), self.MinHumanVel))
				end
			else
				self.Target = nil
				local phys = self:GetPhysicsObject()
...
function ENT:TraceManhack()
	...
	if !self.Target then
		...
		if IsValid(phys) and IsValid(target) and string.find(target:GetClass(), "manhack") or string.find(target:GetClass(), "prop_*drone*") then
			local tpos = target:LocalToWorld(target:OBBCenter())
			local dist = tpos:Distance(pos)
			local dir = (tpos - pos):GetNormal()
